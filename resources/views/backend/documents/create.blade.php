...
<div class="container-fluid">
    ...
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item active">@lang('dashboard.create')</li>
    </ol>
    <div class="card mb-4">
        <div class="card-header"><i class="fas fa-table mr-1"></i>@lang('dashboard.create')</div>
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data" action="{{ route('dashboard.documents.store') }}">
                @csrf
                @if (session('success'))
                    ...
                    <div class="col-md-6">
                        <select name="coupon_id" id="coupon_id" class="form-control @error('coupon_id') is-invalid @enderror">
                            ...
                            @foreach($coupons as $coupon)
                                <option value="{{ $coupon->id }}" {{ old('coupon_id') == $coupon->id ? "selected" : "" }}>{{ $coupon->code }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <input id="file" value="{{ old('file') }}" type="file" class="form-control @error('file') is-invalid @enderror" name="file" required autocomplete="file" autofocus>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <select name="is_provision" id="is_provision" class="form-control @error('is_provision') is-invalid @enderror">
                            <option value="0" {{ old('is_provision') == 0 ? "selected" : "" }}>No</option>
                            <option value="1" {{ old('is_provision') == 1 ? "selected" : "" }}>Yes</option>
                        </select>
